    ...
    while(!quit){
        ...
        if(synchro==1){
			...
			switch(gbuffer[0]){
				...
				case 'I':
					// On récupère l'id fourni par le serveur
				...
				case 'L':
					// On récupère la liste de joueurs
				...
				case 'D':
					// On récupère les 3 cartes distribuées par le serveur
				...
				case 'M':
					// On récupère l'indice du joueur dont c'est le tour
					sscanf(gbuffer, "M %d", &joueurCourant); 
					// On l'affiche
					printf("Joueur Courant: %d\n", joueurCourant); 
					// Si c'est le tour du joueur, alors on peut rendre actif le bouton 'go'
				...
				case 'V':
					/**
					 * Le serveur envoie les 3 informations suivantes: 
					 * - son numéro de joueur.
					 * - l'indice de colonne du symbole.
					 * - la valeur dans la case.
					 */
					// On récupère ces 3 informations
					sscanf(gbuffer, "V %d %d %d", &joueurCourant, &symbole, &value);
					// On les affiche au joueur
					printf("Numéro de joueur: %d\n", joueurCourant);
					printf("Symbole: %d\n", symbole);
